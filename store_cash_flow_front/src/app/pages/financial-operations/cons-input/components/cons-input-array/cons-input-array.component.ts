...
@Component({
    ...
    templateUrl: './cons-input-array.component.html',
    styleUrl: './cons-input-array.component.scss'
})
export class ConsInputArrayComponent implements OnChanges {
    ...
    @Input() dataList$?: Observable<ConsInput[]>;
    sourceData: ConsInput[] = [];
    data: ConsInput[] = [];
    dataTableIsShown: boolean = false;
    imagesMinIoUrlMaps: {[key: string]: string} = {};
    
    clickedMenuLine?: ConsInput;
    selectedLine?: ConsInput;
    
    dataIsLoading = true;
    cancelConsInputDialogIsVisible: boolean = false;
    ...
    @Output() onAddConsInputClick: EventEmitter<boolean> = new EventEmitter();
    ...
    initAttributsDetails() {        
        this.attributsDetailsMap = {
            ...
            receiptSrc: {libelle: 'Receipt image', type: 'image'}
        }
    ...
    getRefTables() {
        ...
        forkJoin([
            ...
            data => {
                // ✅ Action après le chargement de toutes les tables de référence
                this.getData();
            }
        );
    }
    ...
    getImagesUrls() {
        let imagesSrcs: string[] = [];
        this.data.forEach(
            (item: any) => {
                for(let code of this.attributesCodes) {
                    if(this.attributsDetailsMap[code].type === 'image' && item[code])
                        imagesSrcs.push(item[code]);
                }
            }
        );
        imagesSrcs = imagesSrcs.sort();
    ...
    initMenuItems() {
        this.items = [
            ...
            },
            {
                id: 'sell',
                label: 'Sell',
                command: () => this.onMenuItemClick('sell')
            },
            {
                id: 'cancel',
                label: 'Cancel',
    ...
    onMenuItemClick(itemName: 'details' | 'sell' | 'cancel' | 'edit') {
        switch (itemName) {
            case 'details':
                break;
            case 'sell':
                break;
            case 'cancel':
                this.cancelConsInputDialogIsVisible = true;

                // this.consInputService.cancelConsInput(this.clickedMenuLine!.id!).subscribe(
                //     data => {
                //         this.getData();
    ...
    toggleMenu(item: ConsInput, event: Event) {
        this.clickedMenuLine = item;
        this.menu.toggle(event);
    }

    onConfirmCancelConsInput() {
        this.cancelDialogIsLoading = true;
        this.consInputService.cancelConsInput(this.clickedMenuLine!.id!)
            .pipe(
                finalize(() => {
                    this.cancelConsInputDialogIsVisible = false;
            ...
            data => {
                this.messageService.add({
                    severity: 'success',
                    summary: 'Consumable Input Cancellation',
                    detail: 'The selected consumable input transaction has been successfully canceled.'
                });
                this.getData();
            }
        );
